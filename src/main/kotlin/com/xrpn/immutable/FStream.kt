...
sealed class FStream<out A: Any> {
    ...
    fun reverseX(): FStream<A> {
        ...
        tailrec fun accrue(xs: FStream<A>, acc: IMStack<() -> A>): IMStack<() -> A> =
            when (xs) {
                is FSNil -> acc
                is FSCons -> accrue(xs.tail(), acc.tibStack<() -> A>()!!.fpush(xs.head, acc))
            }
    ...
    fun take(n: Int): FStream<A> {
        ...
        tailrec fun accrue(iter: Int, current: FStream<A>, acc: IMStack<() -> A>): IMStack<() -> A>  =
            when {
                ...
                else -> accrue(iter+1, current.tail(), acc.tibStack<() -> A>()!!.fpush((current as FSCons).head, acc))
            }
    ...
    fun takeWhile_a(p: (A) -> Boolean): FStream<A> {
        ...
        tailrec fun accrue(current: FStream<A>, acc: IMStack<() -> A>): IMStack<() -> A>  =
            when {
                current is FSNil -> acc
                p(current.head()!!) -> accrue(current.tail(), acc.tibStack<() -> A>()!!.fpush((current as FSCons).head, acc))
                else -> acc
            }
        ...
        tailrec fun traverseToMatch(
            ...
            when (pos) {
                ...
                is FSCons -> when {
                    ...
                    else -> traverseToMatch(match, pos.tail(), acc.tibStack<() -> A>()!!.fpush(pos.head, acc))
                }
            }
