...
private val tkvInt1cr = TKVEntry.ofkkc(intKeyOf(1), 1, reverseIntCompare)
private val tkvInt2cr = TKVEntry.ofkkc(intKeyOf(2), 2, reverseIntCompare)
private val tkvStr1cr = TKVEntry.ofkvc(strKeyOf(1), 1, reverseStrCompare)
private val tkvStr2cr = TKVEntry.ofkvc(strKeyOf(2), 2, reverseStrCompare)
...
private val tkvIntAcr = TKVEntry.ofkvc(intKeyOf("A"), "A", reverseIntCompare)
private val tkvIntBcr = TKVEntry.ofkvc(intKeyOf("B"), "B", reverseIntCompare)
private val tkvStrAcr = TKVEntry.ofkkc(strKeyOf("A"), "A", reverseStrCompare)
private val tkvStrBcr = TKVEntry.ofkkc(strKeyOf("B"), "B", reverseStrCompare)
...
class FTKVentryTest : FunSpec({
    ...
    test("comparator equals intk intv ") {
        ...
        tkvInt1cr.equals(TKVEntry.ofkkc(intKeyOf(1), 1, reverseIntCompare)) shouldBe true
        tkvInt1cr.equals(TKVEntry.ofIntKey(1)) shouldBe true
        TKVEntry.ofkkc(intKeyOf(1), 1, reverseIntCompare).equals(tkvInt1cr) shouldBe true
        ...
        tkvInt1cr.equals(TKVEntry.ofkkc(intKeyOf(2), 2, reverseIntCompare)) shouldBe false
        val ex1 = shouldThrow<IllegalStateException> {
            tkvInt1cr.equals(TKVEntry.ofIntKey(2))
        }
        ex1.message shouldBe TKVEntryType.CANNOT_COMPARE
        TKVEntry.ofkkc(intKeyOf(2), 2, reverseIntCompare).equals(tkvInt1cr) shouldBe false
        val ex2 = shouldThrow<IllegalStateException> {
            TKVEntry.ofIntKey(2).equals(tkvInt1cr)
        }
        ex2.message shouldBe TKVEntryType.CANNOT_COMPARE
    }

    test("comparator equals intk strv ") {
        tkvIntAcr.equals(null) shouldBe false
        tkvIntAcr.equals(tkvIntAcr) shouldBe true
        tkvIntAcr.equals(TKVEntry.ofkvc(intKeyOf("A"), "A", reverseIntCompare)) shouldBe true
        tkvIntAcr.equals(TKVEntry.ofIntKey("A")) shouldBe true
        TKVEntry.ofkvc(intKeyOf("A"), "A", reverseIntCompare).equals(tkvIntAcr) shouldBe true
        ...
        tkvIntAcr.equals(TKVEntry.ofkvc(intKeyOf("B"), "B", reverseIntCompare)) shouldBe false
        val ex1 = shouldThrow<IllegalStateException> {
            tkvIntAcr.equals(TKVEntry.ofIntKey("B"))
        }
        ex1.message shouldBe TKVEntryType.CANNOT_COMPARE
        TKVEntry.ofkvc(intKeyOf("B"), "B", reverseIntCompare).equals(tkvIntAcr) shouldBe false
        val ex2 = shouldThrow<IllegalStateException> {
    ...
    test("comparator > intk intv ") {
        ...
        (tkvInt1cr > TKVEntry.ofkkc(intKeyOf(2), 2, reverseIntCompare)) shouldBe true
        val ex1 = shouldThrow<IllegalStateException> {
            tkvInt2 > tkvInt1cr
        }
        ex1.message shouldBe TKVEntryType.CANNOT_COMPARE
        ...
        (tkvInt1cr > TKVEntry.ofkkc(intKeyOf(1), 1, intCompare)) shouldBe false
        (TKVEntry.ofIntKey(1) > tkvInt1cr) shouldBe false
        (TKVEntry.ofkkc(intKeyOf(1), 1, intCompare) > tkvInt1cr) shouldBe false
    ...
    test("comparator > strk strvv") {
        ...
        (tkvStrAcr > TKVEntry.ofkkc(strKeyOf("B"), "B", reverseStrCompare)) shouldBe true
    ...
    test("comparator > intk strv ") {
        ...
        (tkvIntAcr > TKVEntry.ofkvc(intKeyOf("B"), "B", reverseIntCompare)) shouldBe true
        val ex1 = shouldThrow<IllegalStateException> {
            tkvIntB > tkvIntAcr
        }
        ex1.message shouldBe TKVEntryType.CANNOT_COMPARE
        ...
        (tkvIntAcr > TKVEntry.ofkvc(intKeyOf("A"), "A", intCompare)) shouldBe false
        (TKVEntry.ofIntKey("A") > tkvIntAcr) shouldBe false
        (TKVEntry.ofkvc(intKeyOf("A"), "A", intCompare) > tkvIntAcr) shouldBe false
