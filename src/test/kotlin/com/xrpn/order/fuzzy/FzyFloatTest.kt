...
import com.xrpn.order.fuzzy.FzyFloat.Companion.zero
...
class FzyFloatTest : FunSpec({
    ...
    test("equal (zeroes)") {
        ...
        floatEps.fzyEqual(zero()) shouldBe true
        zero().equals(floatEps) shouldBe true
    }
    ...
    test("Float.fzyEqual qty pathology") {
        checkAll(Arb.float()) { nf1 ->
            val nf1fzy = nf1.asFzyFloat()
            nf1.fzyEqual(nf1fzy) shouldBe !nf1.isNaN()
            nf1.fzyEqual(nf1.asFzyFloat(alterFloatTol)) shouldBe !nf1.isNaN()
            nf1.fzyEqual(nf1, defaultFloatTol) shouldBe !nf1.isNaN()
            nf1.fzyEqual(nf1, alterFloatTol) shouldBe !nf1.isNaN()
    ...
    test("Float.fzyEqual tol pathology") {
        checkAll(Arb.numericFloats(), Arb.float()) { nfQty, nfTol ->
            ...
            nfQty.fzyEqual(nfQty.asFzyFloat(alterFloatTol)) shouldBe true
