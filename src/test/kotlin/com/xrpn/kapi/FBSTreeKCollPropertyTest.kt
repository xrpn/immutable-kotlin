...
class FBSTreeKCollPropertyTest : FunSpec({
  ...
  test("equals") {
    Arb.fbstreeAsCollection<Int, Int>(Arb.int()).checkAll(repeats) { fbst ->
      val l = FBSTree.of(fbst.iterator())
      val m = FRBTree.of(fbst.iterator())
      ...
      val iter = @Suppress("UNCHECKED_CAST") (fbst.iterator() as? FTreeIterator<Int,Int>)
      iter?.retriever?.original()?.equals(l) shouldBe true
      l.equals(iter?.retriever?.original()) shouldBe true
      iter?.retriever?.original()?.equals(m) shouldBe false
      m.equals(iter?.retriever?.original()) shouldBe false
      iter?.retriever?.original()?.softEqual(m) shouldBe true
      m.softEqual(iter?.retriever?.original()) shouldBe true
    }
    Arb.fbstreeAsCollection<Char, Char>(Arb.char()).checkAll(repeats) { fbst ->
      ...
      val iter = @Suppress("UNCHECKED_CAST") (fbst.iterator() as? FTreeIterator<Char,Char>)
      iter?.retriever?.original()?.equals(l) shouldBe true
      l.equals(iter?.retriever?.original()) shouldBe true
      iter?.retriever?.original()?.equals(m) shouldBe false
      m.equals(iter?.retriever?.original()) shouldBe false
    ...
    Arb.fbstreeAsCollection<String, String>(Arb.string(0..10)).checkAll(repeats) { fbst ->
      ...
      val iter = @Suppress("UNCHECKED_CAST") (fbst.iterator() as? FTreeIterator<String,String>)
      iter?.retriever?.original()?.equals(l) shouldBe true
      l.equals(iter?.retriever?.original()) shouldBe true
      iter?.retriever?.original()?.equals(m) shouldBe false
      m.equals(iter?.retriever?.original()) shouldBe false
  ...
  test("zip array") {
    checkAll(5, Arb.fbstreeAsCollection<Int, Int>(Arb.int()), Arb.fbstreeAsCollection<Int, Int>(Arb.int())) { fbst1, fbst2  ->
      val l1: List<TKVEntry<Int, Int>> = fbst1.toList()
      val a2: Array<TKVEntry<Int, Int>> = FBSTree.toArray(FBSTree.of(fbst2.iterator()))
